...
if __name__ == "__main__":
    ...
    parser.add_argument("-in_f_dir", "--in_f_dir",
                        type=str, default="./examples/front")
    parser.add_argument("-in_b_dir", "--in_b_dir",
                        type=str, default="./examples/back")
    parser.add_argument("-out_dir", "--out_dir", type=str, default="./results")
    parser.add_argument("-seg_dir", "--seg_dir", type=str, default=None)
    parser.add_argument("-cfg", "--config", type=str,
                        default="./configs/econ.yaml")
    ...
    cfg_show_list = [
        "test_gpus", [
            args.gpu_device], "mcube_res", 512, "clean_mesh", True, "test_mode", True,
        "batch_size", 1
    ]
    ...
    if cfg.bni.use_ifnet:
        ...
        print(colored(
            f"Resume IF-Net+ from {Format.start} {cfg.ifnet_path} {Format.end}", "green"))
        print(colored(
            f"Complete with {Format.start} IF-Nets+ (Implicit) {Format.end}", "green"))
    else:
        print(colored(
            f"Complete with {Format.start} SMPL-X (Explicit) {Format.end}", "green"))
    ...
    for data, data2 in pbar:
        ...
        if osp.exists(smpl_path):
            ...
            for idx in range(N_body_f):
                ...
                smpl_verts_f = torch.tensor(
                    smpl_mesh_f.vertices).to(device).float()
                smpl_faces_f = torch.tensor(
                    smpl_mesh_f.faces).to(device).long()
            ...
            for idx in range(N_body_f):
                ...
                smpl_verts_b = torch.tensor(
                    smpl_mesh_b.vertices).to(device).float()
                smpl_faces_b = torch.tensor(
                    smpl_mesh_b.faces).to(device).long()
            ...
            in_tensor_f["T_normal_F"], in_tensor_f["T_normal_B"] = dataset.render_normal(
                batch_smpl_verts_f, batch_smpl_faces
            )

            # render optimized mesh as normal [-1,1]
            in_tensor_b["T_normal_F"], in_tensor_b["T_normal_B"] = dataset.render_normal(
                batch_smpl_verts_b, batch_smpl_faces
            )
            ...
            for i in loop_smpl:
                ...
                smpl_verts_b, smpl_landmarks_b, smpl_joints_b = dataset.smpl_model(
                    ...
                    left_hand_pose=tensor2variable(
                        data2["left_hand_pose"], device),
                    right_hand_pose=tensor2variable(
                        data2["right_hand_pose"], device),
                )


                smpl_verts_f = (smpl_verts_f + optimed_trans_f) * data["scale"]
                smpl_joints_f = (smpl_joints_f + optimed_trans_f) * data["scale"] * torch.tensor([
                    1.0, 1.0, -1.0
                ]).to(device)

                smpl_verts_b = (smpl_verts_b + optimed_trans_b) * data2["scale"]
                smpl_joints_b = (smpl_joints_b + optimed_trans_b) * data2["scale"] * torch.tensor([
                    1.0, 1.0, -1.0
                ]).to(device)
                ...
                ghum_lmks_f = data["landmark"][:, SMPLX_object.ghum_smpl_pairs[:, 0], :2].to(
                    device)
                ghum_conf_f = data["landmark"][:,
                                             SMPLX_object.ghum_smpl_pairs[:, 0], -1].to(device)
                smpl_lmks_f = smpl_joints_3d_f[:,
                                           SMPLX_object.ghum_smpl_pairs[:, 1], :2]
                ...
                ghum_lmks_b = data2["landmark"][:, SMPLX_object.ghum_smpl_pairs[:, 0], :2].to(
                    device)
                ghum_conf_b = data2["landmark"][:,
                                             SMPLX_object.ghum_smpl_pairs[:, 0], -1].to(device)
                smpl_lmks_b = smpl_joints_3d_b[:,
                                           SMPLX_object.ghum_smpl_pairs[:, 1], :2]
                ...
                loose_str = ''.join(
                    [str(j) for j in cloth_overlap_flag.int().tolist()])
                occlude_str = ''.join(
                    [str(j) for j in body_overlap_flag.int().tolist()])
                pbar_desc += colored(
                    f"| loose:{loose_str}, occluded:{occlude_str}", "yellow")
                ...
                if (i == args.loop_smpl - 1) and (not args.novis):
                    ...
                    per_data_lst.append(
                        get_optim_grid_image(
                            per_loop_lst, None, nrow=N_body_f * 2, type="smpl")
                    )
            ...
            in_tensor_f["smpl_verts"] = smpl_verts_f * \
                torch.tensor([1.0, 1.0, -1.0]).to(device)
            in_tensor_f["smpl_faces"] = in_tensor_f["smpl_faces"][:, :, [0, 2, 1]]

            if not args.novis:
                per_data_lst[-1].save(
                    osp.join(args.out_dir, cfg.name,
        ...
        for idx in range(N_body_f):
            ...
            smpl_obj_f = trimesh.Trimesh(
                in_tensor_f["smpl_verts"].detach().cpu()[idx] *
            ...
            if not osp.exists(smpl_obj_path):
                ...
                smpl_info = {
                    ...
                    rotation_matrix_to_angle_axis(
                        data["jaw_pose"][idx]).cpu().unsqueeze(0),
                    "left_hand_pose":
                    rotation_matrix_to_angle_axis(
                        data["left_hand_pose"][idx]).cpu().unsqueeze(0),
                    "right_hand_pose":
                    rotation_matrix_to_angle_axis(
                        data["right_hand_pose"][idx]).cpu().unsqueeze(0),
                    "scale":
                    data["scale"][idx].cpu(),
                }
                np.save(
        ...
        batch_smpl_verts_f = in_tensor_f["smpl_verts"].detach() * torch.tensor([1.0, -1.0, 1.0],
                                                                           device=device)
        batch_smpl_faces_f = in_tensor_f["smpl_faces"].detach()[
            :, :, [0, 2, 1]]
        ...
        for idx in range(N_body_f):
            ...
            in_tensor_f["body_verts"].append(
                torch.tensor(smpl_obj_lst[idx].vertices).float())
            in_tensor_f["body_faces"].append(
                torch.tensor(smpl_obj_lst[idx].faces).long())
            ...
            if cfg.bni.use_ifnet:
                ...
                side_mesh = apply_face_mask(
                    side_mesh, ~SMPLX_object.smplx_eyeball_fid_mask)
                ...
                with torch.no_grad():
                    sdf = ifnet.reconEngine(
                        netG=ifnet.netG, batch=in_tensor_f)
                ...
                side_mesh = register(
                    BNI_object.F_B_trimesh, sm(side_mesh), device)

            side_verts = torch.tensor(
                side_mesh.vertices).float().to(device)
            ...
            if "face" in cfg.bni.use_smpl:
                ...
                # only face
                face_mesh = apply_vertex_mask(
                    face_mesh, SMPLX_object.front_flame_vertex_mask)

                if not face_mesh.is_empty:
                    face_mesh.vertices = face_mesh.vertices - \
                        np.array([0, 0, cfg.bni.thickness])
                    ...
                    face_mesh.export(
                        f"{args.out_dir}/{cfg.name}/obj/{data['name']}_{idx}_face.obj")
                    full_lst += [face_mesh]

            if "hand" in cfg.bni.use_smpl:
                hand_mask = torch.zeros(
                    SMPLX_object.smplx_verts.shape[0], )
                ...
                if data['hands_visibility'][idx][0]:
                    ...
                    hand_mask.index_fill_(
                        0, torch.tensor(mano_left_vid), 1.0)
                ...
                if data['hands_visibility'][idx][1]:
                    ...
                    hand_mask.index_fill_(
                        0, torch.tensor(mano_right_vid), 1.0)
                ...
                if not hand_mesh.is_empty:
                    ...
                    hand_mesh.export(
                        f"{args.out_dir}/{cfg.name}/obj/{data['name']}_{idx}_hand.obj")
                    full_lst += [hand_mesh]
            ...
            side_mesh.export(
                f"{args.out_dir}/{cfg.name}/obj/{data['name']}_{idx}_side.obj")
            ...
            if not args.novis:
                ...
                rotate_recon_lst = dataset.render.get_image(
                    cam_type="four")
                per_loop_lst.extend(
                    [in_tensor_f['image'][idx:idx + 1]] + rotate_recon_lst)
        ...
        if len(per_loop_lst) > 0 and (not args.novis):
            ...
            per_data_lst[-1].save(osp.join(args.out_dir,
                                  cfg.name, f"png/{data['name']}_cloth.png"))

            # for video rendering
            in_tensor_f["BNI_verts"].append(
                torch.tensor(final_mesh.vertices).float())
            in_tensor_f["BNI_faces"].append(
                torch.tensor(final_mesh.faces).long())

            os.makedirs(
                osp.join(args.out_dir, cfg.name, "vid"), exist_ok=True)
            in_tensor_f["uncrop_param"] = data["uncrop_param"]
            in_tensor_f["img_raw"] = data["img_raw"]
            torch.save(
                in_tensor_f, osp.join(
                    args.out_dir, cfg.name, f"vid/{data['name']}_in_tensor.pt")
            )
