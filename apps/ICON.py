...
from apps.HGPIFuNet import HGPIFuNet
...
class ICON(pl.LightningModule):
    ...
    # In your main ICON model class (e.g., apps/ICON.py)

    def test_multiview(self, in_tensor_list, calibs_list):
        """
        Generates a mesh from multi-view inputs.
        """
        self.netG.eval()
        self.netG.training = False
        
        # 1. Extract feature maps for each view
        features_list = []
        with torch.no_grad():
            for in_tensor in in_tensor_list:
                features_G, _ = self.netG.filter(in_tensor, return_inter=True)
                features_list.append(features_G)
        
        # 2. To handle multi-view, we replace the network's query function
        #    with our new query_multiview method temporarily.
        original_query_func = self.netG.query
        self.netG.query = self.netG.query_multiview
        
        # 3. The reconEngine is configured to call netG.query. 
        #    We need to pass our lists of features and calibs to it.
        #    The engine will pass these lists to our patched query_multiview function.
        sdf = self.reconEngine(
            opt=self.cfg, 
            netG=self.netG, 
            features=features_list, 
            proj_matrix=calibs_list 
        )
        
        # 4. Restore the original query function after use
        self.netG.query = original_query_func
