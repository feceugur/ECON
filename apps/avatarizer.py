...
# --------------------------------------------------------------------
# 68‑LANDMARK EXTRACTOR —  GEOMETRIC, HUNGARIAN, DETERMINISTIC
# --------------------------------------------------------------------
...
def extract_68_landmarks(
    econ_tpose   : trimesh.Trimesh,
    smpl_model   : smplx.SMPLX,
    smpl_params  : dict,          # betas, expression, jaw_pose, global_orient, transl, scale
    device       : torch.device,
    k_candidates : int = 8,       # nearest vertices considered for each landmark
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Returns 68 unique, symmetric landmark vertex indices + XYZ coords
    on the ECON T‑pose mesh.  Works even if template ordering is lost.
    """
    print("⇢ Extracting 68 landmarks via Hungarian assignment…")

    # ---------------------------------------------------------------
    # 1. Forward SMPL‑X once in canonical pose (T‑pose body, posed face)
    # ---------------------------------------------------------------
    smpl_model = smpl_model.to(device).eval()
    betas      = smpl_params["betas"].to(device)
    expr       = smpl_params["expression"].to(device)
    jaw_pose   = smpl_params["jaw_pose"].to(device)
    ...
    with torch.no_grad():
        out = smpl_model(
            ...
            body_pose      = torch.zeros((1, smpl_model.NUM_BODY_JOINTS * 3), device=device),  # ← fixed
            jaw_pose       = jaw_pose,
            return_verts   = True,
            return_full_pose = True,
        )

    verts     = out.vertices             # (1,V,3)  GPU
    full_pose = out.full_pose            # (1,J*3)  GPU
    faces_gpu = smpl_model.faces_tensor.to(device)

    # 1a. 51 static vertices
    static_v = faces_gpu[smpl_model.lmk_faces_idx]           # (51,3)
    w_static = smpl_model.lmk_bary_coords.to(device)         # (51,3)
    v_static = static_v[
        torch.arange(51, device=device),
        torch.argmax(w_static, dim=1)
    ]

    # 1b. 17 dynamic vertices
    neck_chain = torch.as_tensor(smpl_model.neck_kin_chain,
                                 dtype=torch.long, device=device)
    dyn_faces_idx, dyn_bary = find_dynamic_lmk_idx_and_bcoords(
        verts, full_pose,
        smpl_model.dynamic_lmk_faces_idx.to(device),
        smpl_model.dynamic_lmk_bary_coords.to(device),
        neck_chain,
    )
    dyn_faces = faces_gpu[dyn_faces_idx.squeeze(0)]
    v_dyn = dyn_faces[
        torch.arange(17, device=device),
        torch.argmax(dyn_bary.squeeze(0), dim=1)
    ]

    # sort the jaw / contour by X in SMPL space (right ➜ left)
    v_all = torch.cat([v_static, v_dyn], dim=0).cpu()        # (68,)
    v_dyn_xyz = verts[0, v_dyn].cpu()
    order_dyn = torch.argsort(v_dyn_xyz[:, 0])
    v_all[51:] = v_all[51:][order_dyn]

    # ---------------------------------------------------------------
    # 2. Transform canonical landmark positions to ECON T‑pose space
    # ---------------------------------------------------------------
    canonical_xyz = verts[0, v_all.to(device)].cpu()         # (68,3)
    R = axis_angle_to_matrix(smpl_params["global_orient"].to(device))[0].cpu()
    posed = (R @ canonical_xyz.T).T
    posed += smpl_params["transl"].cpu()
    posed *= smpl_params["scale"].cpu()
    posed *= torch.tensor([1.0, -1.0, -1.0])                 # axis flip
    posed_np = posed.numpy()                                 # (68,3)

    # ---------------------------------------------------------------
    # 3. For each landmark get its k nearest ECON vertices
    # ---------------------------------------------------------------
    econ_verts = np.asarray(econ_tpose.vertices)
    tree       = cKDTree(econ_verts)
    dists, candidates = tree.query(posed_np, k=k_candidates)   # (68,k)

    unique_verts = np.unique(candidates)
    Vcand        = len(unique_verts)
    vert_col     = {v:i for i,v in enumerate(unique_verts)}

    cost = np.full((68, Vcand), 1e6, dtype=np.float64)
    for r in range(68):
        for j in range(k_candidates):
            v = candidates[r, j]
            cost[r, vert_col[v]] = dists[r, j]

    # ---------------------------------------------------------------
    # 4. Hungarian assignment → unique vertices
    # ---------------------------------------------------------------
    rows, cols = linear_sum_assignment(cost)
    landmark_indices = unique_verts[cols]          # (68,)
    landmark_coords  = econ_verts[landmark_indices]

    print("✓ 68 landmarks assigned uniquely and symmetrically.")
...
smpl_model = smplx.create(
    ...
    use_face_contour=True, use_pca=False, create_expression=False, create_betas=False,
...
# --- CRITICAL CHANGE: Build a reverse map from SMPL-X to ECON DA-pose ---
# We build a k-d tree on the ECON mesh this time.
econ_da_tree = cKDTree(econ_da.vertices)
# For each vertex on the SMPL-X DA mesh, find its nearest neighbor on the ECON DA mesh.
# This creates a complete map: every SMPL-X vertex has a corresponding ECON vertex.
_, smplx_to_econ_map = econ_da_tree.query(smpl_da.vertices)
# --- END CRITICAL CHANGE ---
...
print("🔎  Extracting landmarks using the new reverse map...")
landmark_indices, landmark_coords = extract_68_landmarks(
    econ_tpose  = econ_tpose,
...
print("✅  Landmarks saved:\n"
      f"   • {osp.join(final_dir, 'landmark_indices.npy')}\n"
      f"   • {osp.join(final_dir, 'landmarks_only.ply')}")
