...
class FaceGraftingConfig:
    ...
    # New parameters for hole boundary smoothing
    hole_boundary_smoothing_iterations: int = 10 # Default to 0 (disabled)
    hole_boundary_smoothing_factor: float = 0.5 # Lambda factor for Laplacian smoothing
...
class FaceGraftingPipeline:
    ...
    def _smooth_main_hole_boundary(self) -> bool:
        """
        Applies Laplacian smoothing to the largest boundary loop of self.body_with_hole_trimesh.
        This is intended to smooth the main hole cut for the face.
        Modifies self.body_with_hole_trimesh.vertices in place.
        """
        if self.config.hole_boundary_smoothing_iterations <= 0:
            logger.info("Skipping hole boundary smoothing as iterations are 0 or less.")
            return True
        ...
        logger.info(f"Attempting to smooth main hole boundary for {self.config.hole_boundary_smoothing_iterations} iterations with factor {self.config.hole_boundary_smoothing_factor}.")
        ...
        if not all_loops_vidx:
            logger.info("No boundary loops found on body_with_hole_trimesh. Skipping smoothing.")
            return True

        # Find the largest boundary loop (heuristic for the main face hole)
        largest_loop_vidx = max(all_loops_vidx, key=len, default=None)

        if largest_loop_vidx is None or len(largest_loop_vidx) < 3:
            logger.info("No suitable (largest) boundary loop found for smoothing.")
            return True
        
        logger.info(f"Identified largest boundary loop with {len(largest_loop_vidx)} vertices for smoothing.")

        # Make a copy of vertices to perform iterative updates
        vertices_copy = self.body_with_hole_trimesh.vertices.copy()
        
        num_loop_verts = len(largest_loop_vidx)

        for iteration in range(self.config.hole_boundary_smoothing_iterations):
            new_positions_for_loop = np.zeros((num_loop_verts, 3))
            for i in range(num_loop_verts):
                current_v_idx = largest_loop_vidx[i]
                prev_v_idx = largest_loop_vidx[(i - 1 + num_loop_verts) % num_loop_verts]
                next_v_idx = largest_loop_vidx[(i + 1) % num_loop_verts]

                current_pos = vertices_copy[current_v_idx]
                prev_pos = vertices_copy[prev_v_idx]
                next_pos = vertices_copy[next_v_idx]
                
                # Laplacian: v_new = v_old + lambda * ( (v_prev + v_next)/2 - v_old )
                laplacian_target = (prev_pos + next_pos) / 2.0
                new_pos = current_pos + self.config.hole_boundary_smoothing_factor * (laplacian_target - current_pos)
                new_positions_for_loop[i] = new_pos
            
            # Update the vertices_copy with the new positions for the next iteration
            for i in range(num_loop_verts):
                vertices_copy[largest_loop_vidx[i]] = new_positions_for_loop[i]
        
        # Apply the smoothed positions back to the actual mesh object
        self.body_with_hole_trimesh.vertices = vertices_copy
        
        # It's good practice to update caches if vertices are modified
        if hasattr(self.body_with_hole_trimesh, '_cache'):
            self.body_with_hole_trimesh._cache.clear()
        
        logger.info(f"Hole boundary smoothing applied for {self.config.hole_boundary_smoothing_iterations} iterations.")
        return True
    ...
    def _determine_hole_faces_and_create_body_with_hole(self) -> bool:
           ...
           MeshCleanProcess._is_mesh_valid_for_concat(self.body_with_hole_trimesh, f"TempBodyWHolePreSplit_V{self.INTERNAL_VERSION_TRACKER}"):
            ...
            if hasattr(self.body_with_hole_trimesh, 'split') and callable(self.body_with_hole_trimesh.split):
                ...
                if components:
                    ...
                    logger.info("Split operation on body-with-hole yielded no components. Using pre-split version.") 
        ...
        # --- START ADDITION FOR HOLE SMOOTHING ---
        if self.body_with_hole_trimesh is not None and self.config.hole_boundary_smoothing_iterations > 0:
            if not self._smooth_main_hole_boundary():
                logger.warning("Hole boundary smoothing step failed or was skipped. Proceeding with unsmoothed boundary.")
            else:
                # If smoothing was successful, it's good to re-validate and potentially re-clean
                # as vertex positions changed.
                if not MeshCleanProcess._is_mesh_valid_for_concat(self.body_with_hole_trimesh, f"BodyWithHole_PostSmooth_V{self.INTERNAL_VERSION_TRACKER}"):
                    logger.warning("Body-with-hole became invalid after smoothing. Attempting to re-clean.")
                    # Revert to pre-smoothing state if cleaning fails
                    body_before_smooth_reclean = self.body_with_hole_trimesh.copy()
                    cleaned_after_smooth = FaceGraftingPipeline._thoroughly_clean_trimesh_object(
                        self.body_with_hole_trimesh.copy(), # Pass a copy
                        "BodyWithHoleAfterSmoothReClean",
                        self.INTERNAL_VERSION_TRACKER
                    )
                    if cleaned_after_smooth is not None:
                        self.body_with_hole_trimesh = cleaned_after_smooth
                        logger.info("Re-cleaning after hole smoothing successful.")
                    else:
                        logger.warning("Re-cleaning after hole smoothing failed. Reverting to state before smoothing attempt for safety.")
                        self.body_with_hole_trimesh = body_before_smooth_reclean # This might still be the one from before _thoroughly_clean_trimesh_object
                                                                              # or the cleaned one if smoothing happened after that.
                                                                              # For simplicity, we assume body_state_before_final_clean is a safer bet if all else fails.
                        if not MeshCleanProcess._is_mesh_valid_for_concat(self.body_with_hole_trimesh, "BodyWithHole_PostSmooth_FallbackToPreSmooth"):
                             logger.error("Mesh is invalid even after reverting from failed post-smoothing clean. This is problematic.")
                             # Fallback to a known good state if possible
                             if body_state_before_final_clean is not None and MeshCleanProcess._is_mesh_valid_for_concat(body_state_before_final_clean, "BodyWHolePreFinalCleanAsUltimateFallback"):
                                 self.body_with_hole_trimesh = body_state_before_final_clean
                             else: # Last resort, use the body before any hole cutting.
                                 self.body_with_hole_trimesh = body_state_before_hole_cut.copy()
                                 logger.warning("Ultimate fallback: using body mesh from BEFORE hole cutting due to issues post-smoothing.")


        # --- END ADDITION FOR HOLE SMOOTHING ---
        ...
        # Debug save (moved after potential smoothing)
        if self.body_with_hole_trimesh is not None and \
           MeshCleanProcess._is_mesh_valid_for_concat(self.body_with_hole_trimesh, f"DebugSaveBodyWithHole_V{self.INTERNAL_VERSION_TRACKER}"):
            
            base_output_name, ext = os.path.splitext(self.output_path)
            output_dir = os.path.dirname(self.output_path)
            if output_dir and not os.path.exists(output_dir):
                os.makedirs(output_dir, exist_ok=True)
            
            debug_body_with_hole_path = f"{base_output_name}_DEBUG_body_with_dilated_hole{ext}"
            try:
                self.body_with_hole_trimesh.export(debug_body_with_hole_path)
                logger.info(f"DEBUG: Saved body with dilated hole to: {debug_body_with_hole_path}")
            except Exception as e_export_debug:
                logger.warning(f"DEBUG: Failed to save body_with_hole_trimesh for debugging: {e_export_debug}")
        elif self.body_with_hole_trimesh is None:
            logger.info("DEBUG: body_with_hole_trimesh is None, cannot save for debugging.")
        else: 
            logger.info(f"DEBUG: body_with_hole_trimesh is invalid, cannot save for debugging. V={len(self.body_with_hole_trimesh.vertices) if hasattr(self.body_with_hole_trimesh, 'vertices') else 'N/A'}, F={len(self.body_with_hole_trimesh.faces) if hasattr(self.body_with_hole_trimesh, 'faces') else 'N/A'}")
    ...
    def _create_loft_stitch_mesh(self) -> Optional[trimesh.Trimesh]:
        """
        Build a “strip” that lofts the body hole to the SMPL-X face boundary using
        *exact* SMPL-X boundary vertices.  The strip contains:
            • N   new vertices   – the aligned body loop
            • M   duplicate verts – the ordered SMPL-X face loop
        Faces are built so the outer ring (toward the face) references those
        duplicate verts; a later `merge_vertices()` welds them to the originals.
        """
        if (self.s_loop_coords_ordered is None or len(self.s_loop_coords_ordered) < 3 or
            ...
            logging.warning("Loft strip: missing or too-short loops.")
            return None

        # ─────────────────────────────────────────────────────────────────────────
        # Body & face loops (keep exact SMPL-X positions – NO resampling/rounding)
        # ─────────────────────────────────────────────────────────────────────────
        b_loop = self.b_loop_coords_aligned          # shape (Nb, 3)
        s_loop = self.s_loop_coords_ordered          # shape (Ns, 3)
        Nb, Ns = len(b_loop), len(s_loop)

        # Map every body vert to the nearest face-loop vert (KD-tree is cheap here)
        nearest_s_idx = cKDTree(s_loop).query(b_loop, k=1)[1]   # length Nb

        # Vertices:  [ body-loop  |  duplicate face-loop ]
        strip_verts = np.vstack((b_loop, s_loop))               # shape (Nb+Ns, 3)

        # ─────────────────────────────────────────────────────────────────────────
        # Build two tris per body edge
        # ─────────────────────────────────────────────────────────────────────────
        faces = []
        for i in range(Nb):
            i_next = (i + 1) % Nb

            # body-side indices (local to strip_verts)
            v0 = i
            v1 = i_next

            # face-side indices (offset by Nb because face verts are after body verts)
            s_curr = nearest_s_idx[i]       + Nb
            s_next = nearest_s_idx[i_next]  + Nb

            # two triangles that make one quad
            faces.append([v0, v1, s_next])
            faces.append([v0, s_next, s_curr])
        ...
        # ─────────────────────────────────────────────────────────────────────────
        strip = trimesh.Trimesh(vertices=strip_verts,
                                faces=faces,
                                process=True)

        if not MeshCleanProcess._is_mesh_valid_for_concat(strip, "LoftStripExactFace"):
            logging.warning("Loft strip failed validation.")
            return None

        # Orient faces consistently (optional – keeps normals pointing outward)
        if strip.face_normals is not None and len(strip.face_normals):
            if (np.dot(strip.face_normals[0],
                    strip.triangles_center[0] - strip.centroid) > 0):
                strip.invert()

        logging.info("Loft stitch mesh (exact face verts) built.")
        return strip


    def _extract_main_boundary_loop_from_body_with_hole(self) -> Optional[np.ndarray]:
        """
        Extracts the main (most relevant to the s_loop) boundary loop's ordered 3D coordinates
        from self.body_with_hole_trimesh.
        This is called after body_with_hole_trimesh might have had its hole boundary smoothed.
        """
        if not self.body_with_hole_trimesh or \
           not MeshCleanProcess._is_mesh_valid_for_concat(self.body_with_hole_trimesh, "BodyWithHoleForLoopExtract"):
        ...
        if self.s_loop_coords_ordered is None or len(self.s_loop_coords_ordered) < 3:
            logger.warning("EXTRACT_MAIN_BWH_LOOP: s_loop_coords_ordered (SMPLX face loop) is missing. Cannot reliably select main body hole loop.")
            # Fallback to just largest if s_loop is missing, though this is less robust
            all_loops_vidx = MeshCleanProcess.get_all_boundary_loops(self.body_with_hole_trimesh, min_loop_len=3)
            if not all_loops_vidx:
                logger.warning("EXTRACT_MAIN_BWH_LOOP: No boundary loops found on body_with_hole_trimesh.")
                return None
            largest_loop_vidx = max(all_loops_vidx, key=len, default=None)
            if largest_loop_vidx is not None and len(largest_loop_vidx) >= 3:
                if largest_loop_vidx.max() < len(self.body_with_hole_trimesh.vertices):
                    logger.info("EXTRACT_MAIN_BWH_LOOP: Selected largest loop as s_loop_coords_ordered was missing.")
                    return self.body_with_hole_trimesh.vertices[largest_loop_vidx]
            logger.warning("EXTRACT_MAIN_BWH_LOOP: Could not select a loop (s_loop missing and largest failed).")
            return None

        logger.info("EXTRACT_MAIN_BWH_LOOP: Extracting main boundary loop from body_with_hole_trimesh...")
        all_loops_vidx = MeshCleanProcess.get_all_boundary_loops(self.body_with_hole_trimesh, min_loop_len=3)
        ...
        logger.info(f"EXTRACT_MAIN_BWH_LOOP: Found {len(all_loops_vidx)} candidate boundary loops on body_with_hole_trimesh.")

        best_loop_coords = None
        min_avg_distance = float('inf')

        s_loop_kdtree = cKDTree(self.s_loop_coords_ordered)

        for loop_vidx in all_loops_vidx:
            if loop_vidx is None or len(loop_vidx) < 3:
                continue
            if loop_vidx.max() >= len(self.body_with_hole_trimesh.vertices):
                logger.warning(f"EXTRACT_MAIN_BWH_LOOP: Skipping a loop due to out-of-bounds vertex indices (max: {loop_vidx.max()}, verts: {len(self.body_with_hole_trimesh.vertices)}).")
                continue

            current_loop_coords = self.body_with_hole_trimesh.vertices[loop_vidx]
            
            # Calculate average distance from this loop's points to the s_loop_coords_ordered
            distances, _ = s_loop_kdtree.query(current_loop_coords, k=1)
            avg_distance = np.mean(distances)

            if avg_distance < min_avg_distance:
                min_avg_distance = avg_distance
                best_loop_coords = current_loop_coords
        
        if best_loop_coords is not None:
            logger.info(f"EXTRACT_MAIN_BWH_LOOP: Selected best loop with {len(best_loop_coords)} vertices (avg dist to s_loop: {min_avg_distance:.4f}).")
            return best_loop_coords
        else:
            logger.warning("EXTRACT_MAIN_BWH_LOOP: Failed to select a suitable main boundary loop from body_with_hole_trimesh.")
            return None
    ...
    def _attempt_body_driven_loft(self) -> bool:
        ...
        # MODIFIED PART: Get body loop from the (potentially smoothed) body_with_hole_trimesh
        b_loop_coords_ordered_pre_align = self._extract_main_boundary_loop_from_body_with_hole()
        
        if b_loop_coords_ordered_pre_align is None or len(b_loop_coords_ordered_pre_align) < 3:
            logger.warning("Cannot proceed with loft: Main body hole loop extraction (from body_with_hole_trimesh) failed.")
            return False
        ...
        body_with_hole_for_concat = self.body_with_hole_trimesh # This has the smoothed hole
        ...
        if len(valid_cs) == 3:
            try:
                ...
                trimesh.constants.tol.merge = 1e-5
    ...
    def _perform_simple_concatenation(self) -> bool:
        ...
        try:
            ...
            self.stitched_mesh_intermediate.merge_vertices(merge_tex=False, merge_norm=False)
...
class MeshCleanProcess:
    ...
    def run_face_grafting_pipeline(
        ...
        hole_boundary_smoothing_iterations=10, # e.g., 10 iterations
        hole_boundary_smoothing_factor=0.5
    ) -> Optional[trimesh.Trimesh]:
...
def main():
    """
    Main function to run the FaceGraftingPipeline as a standalone script for testing.
    Loads meshes from the specified directory and processes them.
    """
    # Define paths based on user's specification
    base_input_dir = "/home/ubuntu/projects/induxr/econ_s/ECON/results/Fulden/IFN+_face_thresh_0.01/econ/obj"
    
    # IMPORTANT: User needs to replace these with actual filenames present in base_input_dir
    ...
    # Example: outputting to a 'grafting_test_results' subdirectory of the input directory
    ...
    body_name = os.path.splitext(body_mesh_filename)[0]
    face_name = os.path.splitext(face_mesh_filename)[0]
    output_filename = f"grafted_{body_name}_with_{face_name}.obj"
    output_path = os.path.join(output_dir, output_filename)

    # Configure logging for this script's execution.
    # The module might have already set up basicConfig if no handlers exist.
    script_logger = logging.getLogger("standalone_graft_test") # Use a specific name for this logger
    
    # Ensure logging is configured. If root logger has no handlers, set up basicConfig.
    if not logging.getLogger().hasHandlers():
        logging.basicConfig(level=logging.INFO, 
                            format='%(asctime)s - %(levelname)s - %(name)s - %(message)s')
    
    # Set level for this specific logger if needed, or rely on root config
    script_logger.setLevel(logging.INFO)


    script_logger.info(f"--- Standalone Face Grafting Test ---")
    script_logger.info(f"Full Body Mesh Path: {full_body_mesh_path}")
    script_logger.info(f"SMPLX Face Mesh Path: {smplx_face_mesh_path}")
    script_logger.info(f"Output Path: {output_path}")

    # Check for existence of input files
    if not os.path.exists(full_body_mesh_path):
        script_logger.error(f"CRITICAL ERROR: Input body mesh not found at '{full_body_mesh_path}'.")
        script_logger.error(f"Please ensure the file '{body_mesh_filename}' exists in '{base_input_dir}' or update 'body_mesh_filename'.")
        return
    if not os.path.exists(smplx_face_mesh_path):
        script_logger.error(f"CRITICAL ERROR: Input SMPLX face mesh not found at '{smplx_face_mesh_path}'.")
        script_logger.error(f"Please ensure the file '{face_mesh_filename}' exists in '{base_input_dir}' or update 'face_mesh_filename'.")
        return

    script_logger.info("Input files found. Proceeding with pipeline...")

    # Call the existing static method from MeshCleanProcess to run the pipeline.
    # This method encapsulates the creation of FaceGraftingConfig and FaceGraftingPipeline.
    # Default configuration values from MeshCleanProcess.run_face_grafting_pipeline's
    # signature will be used unless overridden here.
    try:
        result_mesh = MeshCleanProcess.run_face_grafting_pipeline(
            ...
            hole_boundary_smoothing_iterations=10, # e.g., 10 iterations
            hole_boundary_smoothing_factor=0.5    
            # --- Example of overriding specific config parameters for testing ---
            # body_simplification_target_faces=15000, # Default is 12000
            # iterative_repair_s1_iters=3,           # Default is 5
            # stitch_method="simple_concatenate",    # If you want to test only simple concat
        )
    except Exception as e:
        script_logger.error(f"An unhandled exception occurred during pipeline execution: {e}", exc_info=True)
        result_mesh = None


    if result_mesh:
        script_logger.info(f"--- Pipeline completed successfully. ---")
        if hasattr(result_mesh, 'vertices') and hasattr(result_mesh, 'faces'):
            script_logger.info(f"Result mesh contains {len(result_mesh.vertices)} vertices and {len(result_mesh.faces)} faces.")
        script_logger.info(f"Output has been saved to: {output_path}")
    else:
        script_logger.error("--- Pipeline execution failed or did not produce a result. ---")
        script_logger.error(f"Please check logs for details. Output might not have been saved to {output_path}.")
