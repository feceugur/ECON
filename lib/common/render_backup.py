...
def query_avatar_color(verts, faces, front_image, back_image, device):
    ...
    front_sample = torch.nn.functional.grid_sample(
        front_image, uv, align_corners=True, padding_mode='border', mode='bilinear',
    )
    ...
    back_sample = torch.nn.functional.grid_sample(
        back_image, uv, align_corners=True, padding_mode='border', mode='bilinear',
    )
...
def query_color(verts, faces, side_verts, side_faces,
                ...
                threshold=0.8):
    """
    Debug version: Paint the side mesh with a solid color (blue) first,
    then for the main mesh, classify vertices based on visibility:
      - Front: Green [0, 255, 0]
      - Back: Red [255, 0, 0]
    The front_image and back_image arguments are ignored.
    
    Returns a color buffer for all vertices (main + side).
    """
    import torch
    import torch.nn.functional as F

    # ------------------------------
    # PART A: SIDE MESH (paint first)
    # ------------------------------
    side_verts = side_verts.float().to(device)
    N_side = side_verts.shape[0]
    # Paint the side mesh with a solid blue.
    side_color = torch.tensor([0.0, 0.0, 255.0], device=device, dtype=torch.float)
    colors_side = side_color.unsqueeze(0).repeat(N_side, 1)  # [N_side, 3]

    # ------------------------------
    # PART B: MAIN MESH (only vertices not already "side")
    # ------------------------------
    verts = verts.float().to(device)   # [N_main, 3]
    faces = faces.long().to(device)     # [M_main, 3]
    N_main = verts.shape[0]

    # Compute visibility using the main mesh geometry.
    # (get_visibility is assumed to be available and returns a binary flag per vertex.)
    xy, z = verts.split([2, 1], dim=1)
    visibility = get_visibility(xy, z, faces[:, [0, 2, 1]]).flatten().to(device)

    # Define masks for front and back vertices.
    front_mask = (visibility > threshold)
    back_mask  = (visibility < (1.0 - threshold))
    # (Note: We are not doing any "in-between" handling here.)

    # Assign solid debug colors:
    front_color = torch.tensor([0.0, 255.0, 0.0], device=device, dtype=torch.float)  # Green
    back_color  = torch.tensor([255.0, 0.0, 0.0], device=device, dtype=torch.float)  # Red

    # Initialize the main mesh color buffer.
    colors_main = torch.zeros((N_main, 3), device=device)
    colors_main[front_mask] = front_color
    # colors_main[back_mask]  = back_color

    # ------------------------------
    # PART C: COMBINE SIDE AND MAIN MESH COLORS
    # ------------------------------
    # Since side and main meshes are provided separately, we simply concatenate.
    colors = torch.cat([colors_side, colors_main], dim=0)  # [N_side + N_main, 3]
